...
func run() throws {
    guard let tracks = try? Track.gatherAllTracks() else {
        throw ToolError("Cannot get tracks from iTunes")
    }

    guard tracks.count > 0 else {
        throw ToolError("No JSON to record")
    }
    ...
    guard let jsonData = try? encoder.encode(tracks) else {
        throw ToolError("Unable to create JSON for \(tracks)")
    }

    guard let jsonString = String(data: jsonData, encoding: .utf8) else {
        throw ToolError("Unable to create JSON string for \(tracks)")
    }
