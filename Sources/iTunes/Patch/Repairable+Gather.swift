...
>(
  ...
  let allHistoricalGuides = try await repository.transformTracks {
    _, tracks in
    createGuide(tracks)
...
private func changes<Guide: Hashable & Identifiable & Sendable, Change: Sendable>(
  ...
  let allKnownGuides = try await repository.transformTracks { _, tracks in
    createGuide(tracks)
...
extension Repairable {
  private func gatherLibraryPatch(_ repository: Repository) async throws -> Patch {
    guard let createCorrection = libraryCorrections[self] else {
      throw RepairableError.missingRepairableCorrection
    }
    return try await identifierCorrections(repository: repository) {
      [createCorrection($0)]
    }
  }
  ...
  private static func gatherLibraryPatches(_ repository: Repository) async throws -> Patch {
    try await identifierCorrections(
      repository: repository,
  ...
  private static func gatherHistoryPatches(_ repository: Repository) async throws -> Patch {
    async let dateAddedPatch = Repairable.replaceDateAddeds.gather(repository: repository)
    async let dateReleasedPatch = Repairable.replaceDateReleased.gather(repository: repository)
    // .replacePlay is still experimental.
    return try await dateAddedPatch.merge(try await dateReleasedPatch).sorted()
  }

  private static func gatherAllPatches(_ repository: Repository) async throws -> Patch {
    async let libraryChanges = try await gatherLibraryPatches(repository)
    async let historyChanges = try await gatherHistoryPatches(repository)
  ...
  func gather(
    ...
    case .replaceDateAddeds:
      return try await historicalIdentifierCorrections(repository: repository) {
    ...
    case .replaceDurations, .replaceComposers, .replaceComments, .replaceAlbumTitle, .replaceYear,
      .replaceTrackNumber, .replaceIdSongTitle, .replaceIdDiscCount, .replaceIdDiscNumber,
      .replaceArtist:
      return try await gatherLibraryPatch(repository)

    case .libraryRepairs:
      return try await Self.gatherLibraryPatches(repository)

    case .historyRepairs:
      return try await Self.gatherHistoryPatches(repository)

    case .allRepairs:
      return try await Self.gatherAllPatches(repository)
    }
  }
}
